...
import { PaystackTokenizeBank, getAccountName, initiateWithdrawal, } from "../services/index.js";
import { TokenGenerator } from "../helpers/index.js";
import { sendPaymentVerificationEmail } from "../utils/index.js";
// import { trusted } from "mongoose";
const verifyAndTokenizeBankAccount = async (accountName, accountNumber, bankCode, customerId, customerModel) => {
    ...
    const tokenized = await PaystackTokenizeBank(accountName, accountNumber, bankCode, customerId, customerModel);
    if (!tokenized) {
...
const accountName = async (req, res) => {
    try {
...
const addBankAccount = async (req, res) => {
    try {
        ...
        const bankAccount = await verifyAndTokenizeBankAccount(accountName, accountNumber, bankCode, customerId, customerModel);
        res.status(StatusCodes.OK).json({
            success: true,
...
const getAllBankPaymentMethods = async (req, res) => {
    try {
        const customerModel = req.query.customerModel;
        let customerId;
        if (customerModel === "Tutor") {
            customerId = req.tutor?.tutorId;
        }
...
const withdrawalFromBalance = async (req, res) => {
    try {
        ...
        const bankPaymentMethod = await BankPaymentMethod.findById(bankPaymentMethodId);
        ...
        // Check balance first
        const customer = customerModel === "Tutor"
        ...
        const fName = customerModel === "Tutor"
            ? customer.fName
            : customer.firstName;
...
const confirmWithdrawal = async (req, res) => {
    ...
    try {
        const response = await initiateWithdrawal(pendingId, customerModel, verificationToken);
        res.json({
...
const resendWithdrawalOtp = async (req, res) => {
    try {
        ...
        if (pending.verificationResendAttempts > 4) {
            return res.status(StatusCodes.BAD_REQUEST).json({
                success: false,
                message: "You can't request otp for a single transaction more than 3 times",
            });
        }
        ...
        const accountNumber = bankPaymentMethod.decryptAccountNumber();
        const customerId = pending.customerModel === "Tutor"
            ? req.tutor?.tutorId
            : req.affiliate?.affiliateId;
        // Check balance first
        const customer = pending.customerModel === "Tutor"
            ? await Tutor.findById(customerId)
            : await Affiliate.findById(customerId);
        const fName = pending.customerModel === "Tutor"
            ? customer.fName
            : customer.firstName;
        if (!customer || !fName) {
            return res.status(StatusCodes.BAD_REQUEST).json({
        ...
        const { finalVerificationToken, verificationToken } = await TokenGenerator();
...
//   }
// };
export { accountName, addBankAccount, getAllBankPaymentMethods, deletePaymentMethod, withdrawalFromBalance, confirmWithdrawal, resendWithdrawalOtp, };
