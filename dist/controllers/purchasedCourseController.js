...
const fetchPaidCoursesUser = async (req, res) => {
    try {
        ...
        const paidCourses = await PurchasedCourse.find({
            ...
            .populate({
            path: "course",
            select: "_id bannerImage title subTitle description rating promoVideoUrl tutor",
            populate: {
                path: "tutor",
                select: "fName lName email profileImage totalStudent totalReviews totalCourses",
            },
        })
            .sort({ purchasedAt: -1 });
...
const fetchSinglePaidCourseUser = async (req, res) => {
    try {
        ...
        const paidCourse = await PurchasedCourse.findOne({
            ...
            path: "course",
            select: "_id title subTitle description objectives requirements targetAudience category subcategory noOfStudents language otherLanguages thumbnail rating totalRating totalUserRated promoVideoUrl priceInNaira priceInDollar priceInPounds tutor",
            populate: {
                path: "tutor",
