...
export async function createLinkInternal(courseId, customerId, customerModel = "Tutor") {
    try {
...
const getAllPromotedCourses = async (req, res) => {
    try {
...
const getTutorAffiliateCourses = async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = 50;
        const skip = (page - 1) * limit;
        ...
        const promotedCourses = await ShortLink.find({ course: courseId })
            .skip(skip)
            .limit(limit)
            .populate("affiliate", "profilePicture firstName lastName userName email");
        if (!promotedCourses || promotedCourses.length === 0) {
            return res.status(StatusCodes.OK).json({
                success: true,
                message: "No Promoted Courses found",
...
export { createShortLink, getShortLink, getAllPromotedCourses, getTutorAffiliateCourses, };
