...
import { DeleteFileFromCloudinary, UploadFileToCloudinary, } from "../helpers/index.js"; // Your cloudinary config file
import { CourseStatus } from "../constants/index.js";
import mongoose from "mongoose";
const createCourse = async (req, res) => {
    try {
        const { bannerImage, title, subTitle, description, objectives, requirements, targetAudience, category, subcategory, language, otherLanguages, priceInNaira, priceInDollar, priceInPounds, thumbnail, promoVideoUrl, allowAffiliate, affiliateCommission, allowQuestions, 
        // numberOfModules,
        // numberOfLessons,
        // totalDuration,
        modules, } = req.body;
        ...
        if (typeof allowAffiliate !== "boolean") {
            return res.status(StatusCodes.BAD_REQUEST).json({
                success: false,
                message: "Invalid value for allowAffiliate. It must be a boolean (true or false).",
            });
        }
        // Upload banner image
        const bannerRes = await UploadFileToCloudinary(bannerImage, {
            folder: "Course",
        ...
        // Upload thumbnail
        const thumbnailRes = await UploadFileToCloudinary(thumbnail, {
            folder: "Course",
            allowedFileTypes: ["image/png", "image/jpeg", "image/gif"],
            maxSizeInMB: 30,
        }, res);
        ...
        // Upload promo video
        const promoVideoRes = await UploadFileToCloudinary(promoVideoUrl, {
            folder: "Course",
        ...
        for (const module of modules) {
            ...
            for (const lesson of module.lessons) {
                ...
                if (lesson.type === "video") {
                    const videoRes = await UploadFileToCloudinary(lesson.videoUrl, {
                        folder: "Course",
                        allowedFileTypes: ["video/mp4", "video/mov", "video/avi"],
                        maxSizeInMB: 150,
                    }, res);
                ...
                if (lesson.resources && lesson.resources.length > 0) {
                    for (const resource of lesson.resources) {
                        const result = await UploadFileToCloudinary(resource, {
                            folder: "Course",
                            allowedFileTypes: ["video/mp4", "video/mov", "video/avi"],
                            maxSizeInMB: 3000000000,
                        }, res);
        ...
        const response = await createLinkInternal(String(newCourse?._id), tutorId, "Tutor");
...
const updateCourse = async (req, res) => {
    try {
        ...
        const tutorId = req.tutor?.tutorId;
        const id = courseId;
        const { bannerImage, title, subTitle, description, objectives, requirements, targetAudience, category, subcategory, language, otherLanguages, priceInNaira, priceInDollar, priceInPounds, thumbnail, promoVideoUrl, allowAffiliate, affiliateCommission, allowQuestions, modules, } = req.body;
        ...
        if (bannerImage && bannerImage !== course.bannerImage) {
            const newBannerUrl = await UploadFileToCloudinary(bannerImage, {
                folder: "Course",
                allowedFileTypes: ["image/png", "image/jpeg", "image/gif"],
                maxSizeInMB: 30,
            }, res);
        ...
        if (thumbnail && thumbnail !== course.thumbnail) {
            const newThumbUrl = await UploadFileToCloudinary(thumbnail, {
                folder: "Course",
                allowedFileTypes: ["image/png", "image/jpeg", "image/gif"],
                maxSizeInMB: 30,
            }, res);
        ...
        if (promoVideoUrl && promoVideoUrl !== course.promoVideoUrl) {
            const newPromoUrl = await UploadFileToCloudinary(promoVideoUrl, {
                folder: "Course",
                allowedFileTypes: ["video/mp4", "video/mov", "video/avi"],
                maxSizeInMB: 3000000000,
            }, res);
        ...
        const existingModuleIds = existingModules.map((m) => String(m._id));
        ...
        for (const mod of safeModules) {
            ...
            for (const incoming of mod.lessons || []) {
                ...
                let resources = incoming.resources || [];
                if (incoming._id && existingLessonIds.includes(String(incoming._id))) {
                    ...
                    if (incoming.videoUrl && incoming.videoUrl !== oldLesson?.videoUrl) {
                        ...
                        videoUrl = await UploadFileToCloudinary(incoming.videoUrl, {
                            folder: "Course",
                            allowedFileTypes: ["video/mp4", "video/mov", "video/avi"],
                            maxSizeInMB: 150,
                        }, res);
                    ...
                    if (incoming.resources &&
                        ...
                        for (const r of incoming.resources) {
                            const up = await UploadFileToCloudinary(r, {
                                folder: "Course",
                                allowedFileTypes: ["video/mp4", "video/mov", "video/avi"],
                                maxSizeInMB: 3000000000,
                            }, res);
...
const fetchAllCoursesUser = async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = 50;
        const skip = (page - 1) * limit;
        const { search, trending, category, minPrice, maxPrice, minRating, maxRating, } = req.query;
        let filter = { $and: [] };
        if (search) {
            filter.$and.push({
                $or: [
        ...
        const courses = await Course.find(filter)
            ...
            .skip(skip)
            .limit(limit)
            .populate("tutor", "fName lName email profileImage totalStudent totalReviews totalCourses")
...
const fetchSingleCourseUser = async (req, res) => {
    try {
        const { courseId } = req.params;
        const course = await Course.findById(courseId)
            .populate("tutor", "fName lName email profileImage totalStudent totalReviews totalCourses")
            .populate("category", "name description")
            .select("-totalEarnings -totalAffiliate -totalEnrollments -promoVideoUrl");
        if (!course) {
            return res.status(StatusCodes.NOT_FOUND).json({
                success: false,
                message: "Course not found",
...
const fetchAllCoursesTutor = async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = 50;
        const skip = (page - 1) * limit;
...
const fetchSingleCourseTutor = async (req, res) => {
    try {
        const { courseId } = req.params;
        const tutorId = req.tutor?.tutorId;
        if (!tutorId) {
...
const fetchAllCoursesAffiliate = async (req, res) => {
    try {
        ...
        let filter = { $and: [{ allowAffiliate: true }] };
        if (search) {
            filter.$and.push({
                $or: [
                    { title: { $regex: search, $options: "i" } },
...
const fetchSingleCourseAffiliate = async (req, res) => {
    try {
        const { courseId } = req.params;
        const affiliateId = req.affiliate?.affiliateId;
        // ✅ Aggregate pipeline
...
export { createCourse, updateCourse, fetchAllCoursesUser, fetchSingleCourseUser, fetchAllCoursesTutor, fetchSingleCourseTutor, fetchAllCoursesAffiliate, fetchSingleCourseAffiliate, };
