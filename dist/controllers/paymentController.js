        ...
        if (event.event === "charge.success" && verifiedStatus === "success") {
            if (metadata.purpose === "card_tokenization") {
                ...
                try {
                    ...
                    console.error("Refund failed:", refundError.response?.data || refundError);
                }
            }
...
const setDefaultPaymentMethod = async (req, res) => {
    try {
        ...
        const updated = await PaymentMethod.findOneAndUpdate({ _id: methodId, customer: userId }, { isDefault: true }, { new: true });
        if (!updated) {
            return res
...
const coursePayment = async (req, res) => {
    try {
        ...
        if (paymentMethodId) {
            ...
            const response = await payWithExistingBankMethod(paymentMethod.authorizationCode, email, coursePriceInKobo, courseId, userId, "course_payment");
...
export { paystackWebhook, addPaymentMethod, setDefaultPaymentMethod, deletePaymentMethod, getAllPaymentMethods, coursePayment, };
