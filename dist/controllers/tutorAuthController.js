...
import { createHash, createTokenTutor, createTutorJWT, sendVerificationEmail, } from "../utils/index.js";
...
const verifyEmail = async (req, res) => {
    try {
        ...
        if (tutor.verificationToken !== createHash(verificationToken) ||
...
const signinTutor = async (req, res) => {
    try {
        ...
        const tokenTutor = createTokenTutor({
...
const finishOnboarding = async (req, res) => {
    try {
        const { email, expertise, location, phoneNumber, profilePicture, certificateImage, } = req.body;
        if (!email) {
            return res.status(StatusCodes.BAD_REQUEST).json({
                success: false,
                message: "Please provide all values",
        ...
        if (certificateImage && certificateImage.startsWith("data:")) {
            ...
            const uploadResult = await UploadFileToCloudinary(certificateImage, {
